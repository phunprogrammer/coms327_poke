...
const int pokemon_db_size = 1093;
pokemon_db pokemon[pokemon_db_size];

const int pokemon_moves_size = 528239;
pokemon_move_db pokemon_moves[pokemon_moves_size];

const int types_size = 19;
char *types[types_size];

const int moves_size = 845;
move_db moves[moves_size];

const int pokemon_species_size = 899;
pokemon_species_db pokemon_species[pokemon_species_size];

const int experience_size = 601;
experience_db experience[experience_size];

const int pokemon_stats_size = 6553;
pokemon_stats_db pokemon_stats[pokemon_stats_size];

const int stats_size = 9;
stats_db stats[stats_size];
...
void db_parse(bool print)
{
  ...
  for (i = 1; i < 899; i++) {
    fgets(line, 800, f);
    pokemon_species[i].id = atoi((tmp = next_token(line, ',')));
    strcpy(pokemon_species[i].identifier, (tmp = next_token(NULL, ',')));
    tmp = next_token(NULL, ',');
    pokemon_species[i].generation_id = *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].evolves_from_species_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].evolution_chain_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].color_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].shape_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].habitat_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].gender_rate =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].capture_rate =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].base_happiness =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].is_baby =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].hatch_counter =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].has_gender_differences =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].growth_rate_id =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].forms_switchable =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].is_legendary =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].is_mythical =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].order =  *tmp ? atoi(tmp) : INT_MAX;
    tmp = next_token(NULL, ',');
    pokemon_species[i].conquest_order = (*tmp != '\n') ? atoi(tmp) : INT_MAX;
  }
  ...
  if (print) {
    ...
    for (i = 1; i < 899; i++) {
      fprintf(f,
              ...
              i2s(pokemon_species[i].id),
              pokemon_species[i].identifier,
              i2s(pokemon_species[i].generation_id),
              i2s(pokemon_species[i].evolves_from_species_id),
              i2s(pokemon_species[i].evolution_chain_id),
              i2s(pokemon_species[i].color_id),
              i2s(pokemon_species[i].shape_id),
              i2s(pokemon_species[i].habitat_id),
              i2s(pokemon_species[i].gender_rate),
              i2s(pokemon_species[i].capture_rate),
              i2s(pokemon_species[i].base_happiness),
              i2s(pokemon_species[i].is_baby),
              i2s(pokemon_species[i].hatch_counter),
              i2s(pokemon_species[i].has_gender_differences),
              i2s(pokemon_species[i].growth_rate_id),
              i2s(pokemon_species[i].forms_switchable),
              i2s(pokemon_species[i].is_legendary),
              i2s(pokemon_species[i].is_mythical),
              i2s(pokemon_species[i].order),
              i2s(pokemon_species[i].conquest_order));
    }
    fclose(f);
  }
